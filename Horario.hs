...
horasTotais horarios mat =
  sum (map intervalo (filter (\h -> matriculaBolsista h == mat) horarios))

escalaBolsistas :: [Horario] -> String -> Scientific -> [String]
escalaBolsistas horarios diaEscala inicio =
  map matriculaBolsista (filter horarioIgual horarios)
  where
    horarioIgual h = dia h == diaEscala && horaInicio h == inicio

verificaCargaHoraria :: [Horario] -> String -> Bool
verificaCargaHoraria horarios matri =
  horasTotais horarios matri == 12.0

bolsistasPorPesquisa :: [Bolsista] -> [Horario] -> String -> String -> [String]
bolsistasPorPesquisa bolsistas horarios diaEscala lp =
  map matriculaBolsista (filter horarioCompativeis horarios)
  where
